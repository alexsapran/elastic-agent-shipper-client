...
func (Benchmark) All(ctx context.Context, outputFile string, count int) error {
	mg.Deps(InstallBenchStat)
	fmt.Println(">> go benchmark:", "Testing")
	if count == 0 {
		count = 10
	}
	p := filepath.Join("pkg", "helpers")
	args := []string{
		"test",
		fmt.Sprintf("-count=%d", count),
		"-bench=.",
		filepath.Join("github.com/elastic/elastic-agent-shipper-client", p, "..."),
		"-run=Bench#",
	}
